(function () {
  ...
  let liveDashboardOverlay = { states: {} };
  ...
  function getGeneratedSignalsForSelection() {
    return generatedCourtSignals
      .filter((signal) => signal.jurisdictionId === selectedStateId && signal.clauseId === selectedClauseId)
      .sort((a, b) => new Date(b.dateFiled || 0) - new Date(a.dateFiled || 0));
  }

  function getGeneratedBillsForSelection() {
    return generatedBillSignals
      .filter((bill) => bill.jurisdictionId === selectedStateId && bill.clauseId === selectedClauseId)
  ...
  function getOverlaySignalsForSelection(bucket) {
    const stateOverlay = liveDashboardOverlay.states?.[selectedStateId];
  ...
  async function loadLiveDashboardOverlay() {
    try {
      ...
      const payload = await response.json();
      liveDashboardOverlay = payload && payload.states ? payload : { states: {} };
    } catch (error) {
      liveDashboardOverlay = { states: {} };
    }
