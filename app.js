(function () {
  const amendmentDatasets = Object.fromEntries(
    Object.entries({
      "14th Amendment": window.monitorData,
      "2nd Amendment": window.secondAmendmentData,
      "5th Amendment": window.fifthAmendmentData
    }).filter(([, dataset]) => dataset && Array.isArray(dataset.states) && dataset.scope)
  );
  ...
  let selectedAmendmentName = amendmentDatasets["14th Amendment"] ? "14th Amendment" : Object.keys(amendmentDatasets)[0];
  let selectedStateId = amendmentDatasets[selectedAmendmentName]?.states?.[0]?.id || "";
  let selectedClauseId = amendmentDatasets[selectedAmendmentName]?.scope?.clauses?.[0]?.id || "";
  ...
  function previewStateOptions(amendmentName) {
    const previewData = amendmentDatasets[amendmentName] || getCurrentData();
    const previewStates = Array.isArray(previewData?.states) ? previewData.states : [];
    const currentChoice = heroStateSelect.value || selectedStateId;

    heroStateSelect.innerHTML = previewStates.map((state) => `<option value="${state.id}">${state.name}</option>`).join("");

    const nextStateId = previewStates.find((state) => state.id === currentChoice)?.id || previewStates[0]?.id || "";
    heroStateSelect.value = nextStateId;
  }
  ...
  function renderClauseTabs() {
    ...
    selectedClauseBrief.innerHTML = `
      <div class="mini-label">Selected Clause/Aspect</div>
  ...
  heroAmendmentSelect.addEventListener("change", () => {
    previewStateOptions(heroAmendmentSelect.value);
  });
